# RustCost

**Lightweight FinOps & Cost Visibility for Kubernetes**

Built with Rust + React + Helm  
Optimized for simplicity, performance, and open-source transparency.

> **Version 1 (MVP) is planned for launch in December 2025**, focusing on core cost metrics for Pods, Nodes, and Namespaces.

---

### Repositories

- **[rustcost-helmchart](https://github.com/rustcost/rustcost-helmchart)** — Helm chart for deploying RustCost on Kubernetes  
- **[rustcost-backend](https://github.com/rustcost/rustcost-backend)** — Rust + Axum backend for metrics collection and FinOps logic  
- **[rustcost-frontend](https://github.com/rustcost/rustcost-frontend)** — React + Tailwind UI for cost visibility and dashboards  

---

### Mission

We aim to build an **open, efficient, and developer-friendly FinOps toolkit** for modern Kubernetes environments.  

---

_⭐ Star the repos, contribute, and join our mission to make Kubernetes cost insight accessible to everyone._
